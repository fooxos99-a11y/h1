...
export function NazemReconciliationRows({ rows = [] }) {
  ...
  const filtered = useMemo(() => rows.filter((row) => (status === 'all' || row.status === status)
    ...
    .sort((a, b) => sort === 'difference' ? Math.abs(Number(b.difference || 0)) - Math.abs(Number(a.difference || 0))
      : sort === 'points' ? Number(b.recordedPoints || 0) - Number(a.recordedPoints || 0)
        : a.studentName.localeCompare(b.studentName, 'ar')), [rows, search, sort, status]);
  ...
  return <section dir="rtl" className="min-w-0 space-y-3" style={{ fontFamily: 'var(--font-ui)' }} aria-label="مطابقة ناظم">
    ...
    {filtered.slice(currentPage * 30, (currentPage + 1) * 30).map((row) => <article key={row.dailyId || `unlinked:${row.teacherId}:${row.studentId}`} className="min-w-0 space-y-2 rounded-xl border border-primary/20 p-3 text-sm">
      ...
      <p>{row.taskType === 'review' ? 'المراجعة' : row.taskType === 'link' ? 'الربط' : row.track === 'mastery' ? 'الإتقان' : 'الحفظ'} — {row.importedFromNazem === 'true' ? 'المصدر: ناظم' : 'المصدر: المنصة / المطابقة'}</p>
